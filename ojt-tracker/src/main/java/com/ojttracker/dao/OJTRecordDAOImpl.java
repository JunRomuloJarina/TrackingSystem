...
public class OJTRecordDAOImpl implements OJTRecordDAO {
    ...
    public OJTRecord save(OJTRecord r) {
        String sql = "INSERT INTO ojt_records (student_id,work_date,time_in,time_out,morning_time_in,morning_time_out,afternoon_time_in,afternoon_time_out,break_hours,total_hours,remarks) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
        try (PreparedStatement p = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) { bind(p,r); p.executeUpdate(); try(ResultSet k=p.getGeneratedKeys()){if(k.next())r.setId(k.getInt(1));} return r; }
        catch(SQLException e){throw new RuntimeException("Failed to save OJT record.",e);}
    }
    public List<OJTRecord> findAll(int studentId) { return query("SELECT * FROM ojt_records WHERE student_id=? ORDER BY work_date DESC,id DESC", studentId); }
    ...
    public void update(OJTRecord r) { String sql="UPDATE ojt_records SET work_date=?,time_in=?,time_out=?,morning_time_in=?,morning_time_out=?,afternoon_time_in=?,afternoon_time_out=?,break_hours=?,total_hours=?,remarks=? WHERE id=?";try(PreparedStatement p=connection.prepareStatement(sql)){p.setString(1,r.getWorkDate().toString());p.setString(2,r.getMorningTimeIn().toString());p.setString(3,r.getAfternoonTimeOut().toString());setTimes(p,r,4);p.setDouble(8,r.getBreakHours());p.setDouble(9,r.getTotalHours());p.setString(10,r.getRemarks());p.setInt(11,r.getId());p.executeUpdate();}catch(SQLException e){throw new RuntimeException("Failed to update OJT record.",e);} }
    ...
    public List<OJTRecord> findRecent(int sid,int limit){try(PreparedStatement p=connection.prepareStatement("SELECT * FROM ojt_records WHERE student_id=? ORDER BY work_date DESC,id DESC LIMIT ?")){p.setInt(1,sid);p.setInt(2,limit);List<OJTRecord> out=new ArrayList<>();try(ResultSet r=p.executeQuery()){while(r.next())out.add(map(r));}return out;}catch(SQLException e){throw new RuntimeException(e);}}
    ...
    private void bind(PreparedStatement p,OJTRecord r)throws SQLException{p.setInt(1,r.getStudentId());p.setString(2,r.getWorkDate().toString());p.setString(3,r.getMorningTimeIn().toString());p.setString(4,r.getAfternoonTimeOut().toString());setTimes(p,r,5);p.setDouble(9,r.getBreakHours());p.setDouble(10,r.getTotalHours());p.setString(11,r.getRemarks());}
    private void setTimes(PreparedStatement p,OJTRecord r,int start)throws SQLException{p.setString(start,r.getMorningTimeIn().toString());p.setString(start+1,r.getMorningTimeOut().toString());p.setString(start+2,r.getAfternoonTimeIn().toString());p.setString(start+3,r.getAfternoonTimeOut().toString());}
    ...
    private LocalTime parse(ResultSet r,String primary,String fallback)throws SQLException{String v=r.getString(primary);if(v==null&&fallback!=null)v=r.getString(fallback);return v==null?null:LocalTime.parse(v);}
}
